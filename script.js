console.log("Welcome to my Tic Tac Toe ")
 let music= new Audio("music.mp3")
 let audioTurn =new Audio("ting.mp3")
 let gameover=new Audio("gameover.mp3")
 let turn = "X";
 let isgameOver=false;

 // Functon to change the turn
 const changeTurn = () =>{
    return turn === "X"? "0" : "X"


 }

 //Function to check for a win

 const checkWin=()=>{
   let boxtexts=document.getElementsByClassName('classbox')

    let wins=
    [
        [0,1,2,5,5,0],
        [3,4,5,5,15,0],
        [6,7,8,5,25,0],
        [0,3,6,-5,15,90],
        [1,4,7,5,15,90],
        [2,5,8,15,15,90],
        [0,4,8,5,15,45],
        [2,4,6,5,15,135],
    ]
    wins.forEach(e =>
    {
       
            if((boxtexts[e[0]].innerText === boxtexts[e[1]].innerText) && (boxtexts[e[2]].innerText === boxtexts[e[1]].innerText) && (boxtexts[e[0]].innerText !== ""))
        {
         document.querySelector('.Info').innerText=boxtexts[e[0]].innerText+" Won "
         isgameOver=true;
         document.querySelector('.img').getElementsByTagName('img')[0].style.width='250px';
         document.querySelector(".line").style.transform = `translate(${e[3]}vw, ${e[4]}vw) rotate(${e[5]}deg)`
        
        
    const windowWidth = window.innerWidth; 
    if(windowWidth>=950)  {
            document.querySelector(".line").style.width = "20vw";
        }
        else
        {
            document.querySelector(".line").style.width = "50vw";
        }
    } 
    })

 }

 // Game Logic
//  music.play();
let boxes=document.getElementsByClassName("box");
Array.from(boxes).forEach(element =>{
    let boxtext=element.querySelector(".classbox")
    element.addEventListener('click',()=>{
        if(boxtext.innerText === '')
        {
            boxtext.innerText=turn;
            turn=changeTurn();
            audioTurn.play();
            checkWin();
            if(! isgameOver){
            document.getElementsByClassName("Info")[0].innerText="Turn for "+turn;
            }
        }
    })
})

// add onclick listener to reset button
reset.addEventListener('click',()=>
{
    let boxtext=document.querySelectorAll('.classbox');
    Array.from(boxtext).forEach(element =>
        {
            element.innerText=""
        });
        turn ="X"
     isgameOver=false;
            document.getElementsByClassName("Info")[0].innerText="Turn for "+turn;
            document.querySelector('.img').getElementsByTagName('img')[0].style.width='0px';
            document.querySelector(".line").style.width = "0vw";


             // Re-enable click event on boxes
  Array.from(boxes).forEach((element) => {
    element.addEventListener("click", boxClick);
  });

  // Re-enable hover effect on boxes
  Array.from(boxes).forEach((element) => {
    element.addEventListener("mouseenter", () => {
      if (!isgameover) {
        element.classList.add("hover-effect");
      }
    });
    element.addEventListener("mouseleave", () => {
      element.classList.remove("hover-effect");
    });
  });
});
  
